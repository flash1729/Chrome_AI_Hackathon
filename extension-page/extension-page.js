...
function renderSessionDetail() {
  ...
  sessionCreated.textContent = `Created: ${formatDate(
...
function renderHistory() {
  ...
  sortedHistory.forEach((item) => {
    ...
    if (item.usedIncremental) {
      badge.textContent = "⚡ Incremental";
      badge.style.color = "#ea8600";
    } else if (item.usedWebResearch) {
      badge.textContent = "🔍 With Research";
      badge.style.color = "#1967d2";
    } else {
      badge.textContent = "✓ Full Build";
    ...
    if (message.type === MessageTypes.OPTIMIZATION_COMPLETE) {
      if (currentSession && message.payload.sessionId === currentSession.id) {
        ...
        if (result.usedIncremental) {
          showSuccess(
            "✨ Prompt extended with new context (incremental optimization)"
          );
        }
